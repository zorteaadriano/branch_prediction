...
const char *bpName[6] = { "Static", "Gshare", "Gshare1bit",
                          "Tournament", "Custom", "Gselect" };
...
void set_state_prediction_1bit(uint8_t outcome, uint32_t* table, uint32_t history){
	if(outcome==NOTTAKEN && table[history]!=0)
	{
		table[history]--;
	}
	else if(outcome==TAKEN && table[history]!=1)
	{
		table[history]++;
	}
//	return table[history];
...
train_predictor(uint32_t pc, uint8_t outcome)
{
	...
	switch(bpType) {
		...
		case GSHARE1BIT:
			...
			index = gval ^ pcval;
			//prediction= global_history_table[index];
			set_state_prediction_1bit(outcome, global_history_table, index);
			global_history = outcome | (global_history<<1);
			return;
		case TOURNAMENT:
			gval= mask_input(global_mask, global_history);
